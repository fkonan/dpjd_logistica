...
class Model {
  ...
  public $id;
  protected static $_db, $_table, $_softDelete=false,$_driver='MySql',$_sistema='B';
  ...
  public static function getDb(){
    if(!self::$_db) {
      self::$_db = DB::getInstance(static::$_driver,static::$_sistema);
    }
    return self::$_db;
  }
   ...
   */
  public static function findById($campo='id',$id) {
    return static::findFirst(['conditions'=>"{$campo} = ?", 'bind' => [$id]]);
  }
   ...
   */
  public function update($fields,$llave='id') {
    if(empty($fields) || $this->id == '') return false;
    return static::getDb()->update(static::$_table,$llave, $this->id, $fields);
  }
   ...
   */
  public function delete($llave='id') {
    if($this->id == '' || !isset($this->id)) return false;
    $this->beforeDelete();
    if(static::$_softDelete) {
      $deleted = $this->update(['estado' => 0]);
    } else {
      $deleted = static::getDb()->delete(static::$_table, $llave,$this->id);
    }
    $this->afterDelete();
    return $deleted;
  }
